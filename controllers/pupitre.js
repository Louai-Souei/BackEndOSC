...
const assignLeadersToPupitre = async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(chefsIds) || chefsIds.length !== 2) {
      return res
        .status(400)
        .json({ error: "Doit fournir exactement deux IDs de chefs" });
    }
    ...
    res
      .status(200)
      .json({ message: "les chefs assignés avec succès", modifiedUsers });
  } catch (error) {
...
const createPupitre = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: `Erreur lors de la création du pupitre : ${error.message}`,
      });
  }
};
const updatePupitreById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: `Erreur lors de la mise à jour du pupitre : ${error.message}`,
      });
  }
};
  ...
  try {
    ...
    for (const pupitre of pupitres) {
      ...
      if (pupitre.choristes.includes(userId)) {
        ...
        return res.status(200).json({ pupitre: pupitre });
      }
    }
    ...
    return res
      ...
      .json({
        success: false,
        message: "L'utilisateur n'est pas associé à un pupitre.",
      });
  } catch (error) {
    return res
      .status(500)
      .json({
        success: false,
        error: `Erreur lors de la récupération de l'ID du pupitre : ${error.message}`,
      });
  }
};
